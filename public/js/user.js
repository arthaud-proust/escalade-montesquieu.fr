...
var Compressor = __webpack_require__(/*! compressorjs */ "./node_modules/compressorjs/dist/compressor.js");
        ...
        while (1) {
          switch (_context.prev = _context.next) {
            case 0:
              ...
              img = document.createElement('img'); // When the event "onload" is triggered we can resize the image.
              ...
              img.onload = function () {
                ...
                var h = this.height,
                    w = this.width; // We resize the image with the canvas method drawImage();
...
$(function () {
  try {
    document.getElementById('img').onchange = function (evt) {
      ...
      reader.onload = function (e) {
        console.log(e);
        resizedataURL(e.target.result).then(function (img) {
          new Compressor(img, {
            quality: 0.4,
            success: function success(result) {
              result.name = "image.jpg";
              document.getElementById("preview").src = URL.createObjectURL(result);
              var formData = new FormData(); // The third parameter is required for server

              formData.append('img', result, result.name); // Send the compressed image file to server with XMLHttpRequest.

              axios.post('/profil/img', formData).then(function (r) {
                console.log(r.data);
              });
            },
  ...
  try {
    ...
    // });
    $('#max_voie').html(cotes.map(function (cote) {
      return "<option ".concat($('#max_voie').attr('data-value') == cote ? 'selected' : '', ">").concat(cote, "</option>");
    }));
    $('#max_bloc').html(cotes.map(function (cote) {
      return "<option ".concat($('#max_bloc').attr('data-value') == cote ? 'selected' : '', ">").concat(cote, "</option>");
    }));
...
module.exports = __webpack_require__(/*! D:\projects\Web\escalade-montesquieu.fr\master\resources\js\user.js */"./resources/js/user.js");
